...
class _TaskFormScreenState extends State<TaskFormScreen> {
  ...
  onPressed: () {
    ...
    // Debugging - check the state of the widget
    print("Task is being saved. Task: $task");

    if (widget.task == null) {
      // Debugging - when adding a new task
      print("Adding new task...");
      context.read<TaskBloc>().add(AddTask(task: task));
    } else {
      // Ensure that widget.index is not null before updating
      if (widget.index == null) {
        print("Error: Index is null, cannot update task.");
      } else {
        // Debugging - when updating an existing task
        print("Updating task at index: ${widget.index}");
        context.read<TaskBloc>().add(UpdateTask(
          index: widget.index!,  // Ensure widget.index is not null here
          task: task,
        ));
      }
    }
    ...
    print("Popping context to go back.");
    Navigator.pop(context);
  },
  child: Text(widget.task == null ? 'Create Task' : 'Update Task'),
)
